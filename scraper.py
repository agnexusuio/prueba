...
def recolectar_pinsoft(page, fuente):
    print("\n[Pinsoft] Extrayendo inventario ordenado por precio ascendente...")
    productos = []
...
def recolectar_wordpress(page, fuente):
    print(f"\n[{fuente['nombre']}] Extrayendo inventario ordenado por precio ascendente...")
    productos = []
    for pagina in range(1, 15):
        ...
        try:
            ...
            for tarjeta in tarjetas:
                ...
                if precio is None or precio < 40:
                    continue
...
def extraer_caracteristicas(nombre):
    ...
    if len(caracteristicas) < 4:
        ...
        for t in tokens:
            ...
            if len(caracteristicas) >= 4:
                break
    return caracteristicas[:4] if caracteristicas else ["Garantía de fábrica", "Disponibilidad inmediata", "Envío a nivel nacional", "Equipo original"]
...
def main():
    ...
    with sync_playwright() as p:
        ...
        for fuente in FUENTES:
            ...
            print(f"\n>> {fuente['nombre']}: {len(seleccionados)} productos más económicos seleccionados.")

            for idx, prod in enumerate(seleccionados, 1):
                print(f"  [{idx:02d}] {prod['nombre']} | Original: ${prod['precio_original']:.2f}")
                try:
    ...
    print("\n" + "=" * 60)
    print(f"✓ Catálogo generado: {len(salida)} productos más económicos.")
